...
insert into users_roles (email,password,role,active) values ('piatrata@yandex.com','1234','admin',true) on conflict (email) do update set password='1234', role='admin', active=true;
insert into users_roles (email,password,role,active) values ('Serghei','1111','operator',true) on conflict (email) do update set password='1111', role='operator', active=true;
...
drop policy if exists "public all users_roles" on users_roles;
drop policy if exists "public all advances" on advances;
drop policy if exists "public all sms_logs" on sms_logs;
create policy "public all workers" on workers for all using (true) with check (true);
create policy "public all users_roles" on users_roles for all using (true) with check (true);
create policy "public all advances" on advances for all using (true) with check (true);
create policy "public all sms_logs" on sms_logs for all using (true) with check (true);
